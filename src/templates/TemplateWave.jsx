          ...
          {(invoice.items || []).map((item, idx) => {
            ...
            return (
              <tr key={item.id} className="tw-tr">
                ...
                <td className="tw-td tw-r">{item.free ? '—' : formatINR(parseFloat(item.rate) || 0)}</td>
                <td className="tw-td tw-r">{item.free ? '—' : `${item.discount || 0}%`}</td>
                <td className="tw-td tw-r">{item.free ? '—' : `${item.gst || 0}%`}</td>
                <td className="tw-td tw-r tw-bold" style={item.free ? { color: '#10b981' } : {}}>{item.free ? 'FREE' : formatINR(c.total)}</td>
              </tr>
            );
          })}
